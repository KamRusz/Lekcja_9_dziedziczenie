...
from os import listdir, path, getcwd, system, name
import json
import pickle
...
class CommonReader:
        ...
        try:
            ...
            if path.exists(self.load_file_path):
                if path.isfile(self.load_file_path):
                    ...
                    quit(F"\nścieżka odczytu nie jest plikiem!\nPoniżej lista plików z katalogu [{path.abspath(self.load_file_path)}]:\n{listdir(self.load_file_path)}\n")
            else:
    ...
    def make_change(self):
        for change in self.changes:
            if self.change_flag:
                if len(self.data)<=int(change[0]) or len(self.data[0])<=int(change[1]):
                    print(f"\nwspółrzedne [{change[0]},{change[1]}] poza zakresem pliku")
                else:
...
class SpecificReader(CommonReader):
    def read_csv(self):
        with open(self.load_file_path,"r", newline="") as self.file:   
            self.reader = csv.reader(self.file, skipinitialspace=True, delimiter=',')
    ...
    def save_json(self):
        with open("json4.json", "w") as self.file:
            json.dump(CommonReader.data, self.file, indent=4, ensure_ascii=False)
    ...
    def save_pickle(self):
        with open("pickle.pickle", "wb") as self.file:
            pickle.dump(CommonReader, self.file, pickle.HIGHEST_PROTOCOL)
...
def main():
    ...
    #dispatcher2 = {"load":{"csv":plik.read_csv, "json":SpecificReader.read_json, "pickle":SpecificReader.read_json},"save":{"csv":SpecificReader.save_csv, "json":SpecificReader.save_json, "pickle":SpecificReader.save_json}}
